...
import lumen.window.Windowing;
    ...
    @:noCompletion typedef LumenWindowSystem = lumen.platform.html5.window.WindowSystemHTML5;

#else 

    #if lumen_window_sdl
        @:noCompletion typedef LumenWindowSystem = lumen.platform.native.window.sdl.WindowSystemSDL;
    #else
        @:noCompletion typedef LumenWindowSystem = lumen.platform.html5.window.WindowSystemNative;
    #end 
...
@:noCompletion class WindowSystem {
    ...
    public function display_count() : Int { return 1; }
        /** Get the number of display modes present */
    public function display_mode_count( display:Int ) : Int { return 1; }
        /** Get the native mode information of the display by index */
    public function display_native_mode( display:Int ) : DisplayMode { return null; }
        /** Get the current mode information of the display by index */
    public function display_current_mode( display:Int ) : DisplayMode { return null; }
        /** Get the information from a specific mode index, the index obtrained from iterating with `display_mode_count` value */
    public function display_mode( display:Int, mode_index:Int ) : DisplayMode { return null; }
        /** Get the bounds of the display by index */
    public function display_bounds( display:Int ) : DisplayBounds { return null; }
        /** Get the name of the display by index, where available */
    public function display_name( display:Int ) : String { return ''; }
