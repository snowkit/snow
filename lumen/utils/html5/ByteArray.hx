...
class ByteArray implements ArrayAccess<Int> {
	...
	private function _lumenResizeBuffer(len:Int):Void {
   ...
   private inline function lumenFromBytes(inBytes:Bytes):Void
   {
	...
	public inline function lumenGet(pos:Int):Int {
	...
	public inline function lumenGetBuffer() {
	...
	public static function lumenOfBuffer(buffer:ArrayBuffer):ByteArray {
	...
	public inline function lumenSet(pos:Int, v:Int):Void {
	...
	private inline function set_length(value:Int):Int {
		...
		if (allocated < value)
			_lumenResizeBuffer(allocated = Std.int(Math.max(value, allocated * 2)));
		else if (allocated > value)
			_lumenResizeBuffer(allocated = value);
		length = value;
		return value;
	}
